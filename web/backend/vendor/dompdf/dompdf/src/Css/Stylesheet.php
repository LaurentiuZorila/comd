...
/**
 * The master stylesheet commonClasses
 *
 * The Stylesheet commonClasses is responsible for parsing stylesheets and style
    ...
    private function _css_selector_to_xpath($selector, $first_pass = false)
    {
        ...
        while ($i < $len) {
            ...
            switch ($s) {
                ...
                case ".":
                case "#":
                    // All elements matching the current token with a commonClasses/id equal to
                    // the _next_ token.

                    $attr = $s === "." ? "commonClasses" : "id";

                    // empty commonClasses/id == *
                    if (mb_substr($query, -1, 1) === "/") {
                        $query .= "*";
                    }

                    // Match multiple classes: $tok contains the current selected
                    // commonClasses.  Search for commonClasses attributes with commonClasses="$tok",
                    // commonClasses=".* $tok .*" and commonClasses=".* $tok"
